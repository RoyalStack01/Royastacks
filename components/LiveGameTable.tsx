...
function parseCard(notation: string): CardFace {
  ...
  const rawRank = notation.slice(0, -1).toUpperCase();
  ...
  return (
    ...
    >
      ...
      <g transform="translate(3,16)">
        <SuitIcon suit={card.suit} size={8.4} color={suitColor} />
      </g>
      <g transform={`translate(${38 / 2 - 9.1},${54 / 2 - 9.1})`}>
        <SuitIcon suit={card.suit} size={18.2} color={suitColor} />
      </g>
...
export default function LiveGameTable({
    ...
    getPool(token, pid)
      .then((pool) => {
        if (pool.status === "CLOSED") {
          ...
          toast(
            "Pool was cancelled on-chain. Returning to lobby...",
            "error",
            4000,
          );
        ...
        if (typeof pool.playerCount === "number")
          setPlayerCount(pool.playerCount);
      })
      .catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    socket.on(
      "PLAYER_LEFT",
      ({ walletAddress: addr }: { walletAddress: string }) => {
    ...
    socket.on("GAME_STATE_UPDATED", (state: GameState) => {
      ...
      if (state.winners && state.winners.length > 0) {
        ...
        if (
          winner.walletAddress?.toLowerCase() === walletAddress.toLowerCase()
        ) {
  ...
  function sendAction(
    ...
    else if (type === "call" || type === "raise" || type === "bet")
      sounds.chipClink();
  }
  ...
  const me = gameState?.players.find(
    (p) => p.walletAddress.toLowerCase() === myAddr,
  );
    ...
    return (
      ...
      >
        ...
        <div style={{ color: "#888", fontSize: 14, marginBottom: 8 }}>
          {playerCount} / 5 players in pool #{poolId}
        </div>
        <div
          style={{
            color: connected ? "#9ceb9c" : "#ff6b6b",
            fontSize: 12,
            letterSpacing: 1,
          }}
        >
      ...
      >
        ...
        <div
          style={{
            ...
            boxShadow: `0 0 0 6px ${CRIMSON}44, 0 0 60px 10px #000a, inset 0 0 80px #0008`,
            overflow: "hidden",
          }}
        >
          <div
        ...
        {sortedPlayers.map((player, index) => {
          ...
          const cards: CardFace[] = player.holeCards
            ? player.holeCards.map(parseCard)
            : [];
          const showCards =
            isHuman ||
            (Array.isArray(gameState.winners) &&
              ...
              >
                ...
                <div style={{ position: "relative" }}>
                  ...
                  >
                    ...
                    >
                      {walletEmoji(player.walletAddress)}
                    </div>
                    {isHuman && (
                      <div
                        style={{
            ...
            isRaising ? (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <button
                    onClick={() => {
                      sendAction(currentBet > 0 ? "raise" : "bet", raiseValue);
                      setIsRaising(false);
                    }}
                    style={{
              ...
              <>
                ...
                <button
                  onClick={() => {
                    setRaiseValue(
                      currentBet > 0
                        ? Math.max(currentBet * 2, currentBet + 20)
                        : 20,
                    );
                    setIsRaising(true);
                  }}
        ...
        {log.length > 0 && (
          ...
          >
            {log.map((entry, i) => (
              <div
                key={i}
                style={{ color: "#666", fontFamily: "monospace", fontSize: 11 }}
              >
                {entry}
              </div>
            ))}
